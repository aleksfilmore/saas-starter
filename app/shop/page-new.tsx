...
import { SHOP_PRODUCTS, formatBytes, formatPrice } from '@/lib/shop/constants'
...
export default function ShopPage() {
  ...
  const handlePurchase = async (product: any, paymentMethod: 'bytes' | 'cash') => {
    if (product.requiresShipping && paymentMethod === 'cash') {
    ...
    try {
      if (paymentMethod === 'bytes') {
        ...
        // Handle cash purchase via PayLink
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-4 pt-4">
                    <Button
                      onClick={() => handlePurchase(heroProduct, 'bytes')}
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-col gap-2">
                        {product.bytePrice === 0 ? (
                          <Button
                            onClick={() => handlePurchase(product, 'bytes')}
                            disabled={!unlockable || isLoading}
                            className="w-full bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800"
                          >
                            {unlockable ? 'Unlock Now' : 'Complete 7-Day Streak'}
                          </Button>
                        ) : (
                          <>
                            <Button
                              ...
                              size="sm"
                            >
                              {affordable ? 'Unlock with Bytes' : `Need ${formatBytes(product.bytePrice - userBytes)} More`}
                            </Button>
                          </>
                        )}
                      </div>
