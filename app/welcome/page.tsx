  ...
  const fetchUserData = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        // If user doesn't have archetype data, redirect to quiz for better onboarding
        if (!userData.user.archetype) {
          router.push('/quiz');
          return;
        }
        
        // Log onboarding analytics for user testing
        if (typeof window !== 'undefined') {
          console.log('Welcome page viewed:', {
            userId: userData.user.id,
            archetype: userData.user.archetype,
