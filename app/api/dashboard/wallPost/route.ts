...
async function handler(_req: NextRequest) {
  try {
    ...
    const existing = await db.execute(sql`SELECT wall_post FROM user_daily_actions WHERE user_id = ${user.id} AND action_date = CURRENT_DATE`);
    if (existing.length === 0) {
      await db.execute(sql`INSERT INTO user_daily_actions (user_id, action_date, wall_post) VALUES (${user.id}, CURRENT_DATE, true)`);
  await awardBytes(user.id, 'daily_wall_post', BYTES_REWARD);
  await emitAnalytics(AnalyticsEvents.DAILY_ACTION_WALL_POST_COMPLETED, { userId: user.id, properties:{ source:'dashboard' } });
      return NextResponse.json({ success: true, completed: true, alreadyCompleted: false });
    }
    if (existing[0].wall_post) {
      return NextResponse.json({ success: true, completed: true, alreadyCompleted: true });
    }
    await db.execute(sql`UPDATE user_daily_actions SET wall_post = true WHERE user_id = ${user.id} AND action_date = CURRENT_DATE`);
  await awardBytes(user.id, 'daily_wall_post', BYTES_REWARD);
  await emitAnalytics(AnalyticsEvents.DAILY_ACTION_WALL_POST_COMPLETED, { userId: user.id, properties:{ source:'dashboard' } });
    return NextResponse.json({ success: true, completed: true, alreadyCompleted: false });
  } catch (e) {
    console.error('wallPost action error', e);
    return NextResponse.json({ error: 'Failed to record wall post action' }, { status: 500 });
  }
}
