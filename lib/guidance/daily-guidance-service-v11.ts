...
export class DailyGuidanceService {
  ...
  private async getUserGuidanceState(userId: string): Promise<UserGuidanceState> {
    try {
      ...
      // Get therapy usage
      const therapyUsage = {
        sessionsThisWeek: 0, // This would need therapy session tracking
        lastSession: undefined
      };

      // Get ritual history
      const recentCompletions = await db
        .select({ ritualId: dailyRitualCompletions.ritual_id })
        .from(dailyRitualCompletions)
        .where(eq(dailyRitualCompletions.user_id, userId))
        .orderBy(desc(dailyRitualCompletions.completed_at))
        .limit(10);
