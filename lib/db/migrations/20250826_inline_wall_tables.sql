-- Inline Wall of Wounds core tables (anonymous_posts, wall_post_reactions, ritual_entries)
-- Drop wall_post_comments since feature is unused.
DO $$
BEGIN
  -- anonymous_posts
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='anonymous_posts' AND column_name='id') THEN
  ...
  -- wall_post_reactions
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='wall_post_reactions' AND column_name='id') THEN
  ...
  -- ritual_entries
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='ritual_entries' AND column_name='id') THEN
  ...
  -- Drop unused wall_post_comments table if present
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='wall_post_comments') THEN
    DROP TABLE wall_post_comments;
  END IF;
END $$;
