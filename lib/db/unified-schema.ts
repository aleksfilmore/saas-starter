// Unified canonical schema (superset of previous schema.ts and minimal-schema.ts)
// Non-destructive initial merge: retains legacy columns; future migrations can prune.
import { pgTable, text, timestamp, integer, boolean, varchar, jsonb, date, serial } from 'drizzle-orm/pg-core';
...
export const users = pgTable('users', {
  ...
  // From minimal-schema profile/gamification
  tier: varchar('tier', { length: 50 }).notNull().default('freemium'),
  archetype: varchar('archetype', { length: 100 }),
  archetype_details: jsonb('archetype_details'),
  ...
  // Legacy / schema.ts fields (kept for compatibility) TODO: deprecate
  username: text('username'),
  avatar: text('avatar'),
  onboardingCompleted: boolean('onboarding_completed').default(false),
  subscriptionTier: text('subscription_tier').default('ghost_mode'),
  xpPoints: integer('xp_points').default(0),
  byteBalance: integer('byte_balance').default(100),
...
export const notificationSchedules = pgTable('notification_schedules', {
  ...
  backoff_seconds: integer('backoff_seconds').notNull().default(0),
  dedupe_window_seconds: integer('dedupe_window_seconds').notNull().default(0),
  created_at: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  updated_at: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
});
